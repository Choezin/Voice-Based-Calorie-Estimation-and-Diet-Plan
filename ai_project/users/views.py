...
def record(request):  
    ...
    data = pd.read_csv(r'C:\Users\sheki\OneDrive\Documents\MoM\AI_project\nutri.csv')
...
def content(request, user_name, flag, rec):
    ...
    rec1 = eval(rec)
    df = pd.read_csv('/Users/junjinbully/Desktop/nutri.csv')
    df = df.dropna()
    ...
    if not rec1.objects.filter(user_name=user_name).exists():
        ...
        recom = features.apply(lambda x: pd.Series(x).dot(user_pro), axis=1)
...
def rules(dict2, user_name):
    ...
    # dict2 = request.session['dict2']
        ...
        df = pd.read_csv('C:\\Users\\sheki\\Downloads\\nutri.csv')
        df = df.dropna()
        df.drop_duplicates(inplace = True)
...
def breakfast(request):
    ...
    if(success == 'true'):
        ...
        if(status == 'delete'):
            ...
            if(len(foods) == 0):  
                return render(request, 'users/breakfast.html', dict2) 
...
def lunch(request):
    ...
    if(success == 'true'):
        ...
        if(status == 'delete'):
            ...
            # request.session['snacks_list'] = foods     
            if(len(foods) == 0):  
                return render(request, 'users/lunch.html', dict2) 
...
def snacks(request):
    ...
    if(success == 'true'):
        ...
        if(status == 'delete'):
            ...
            # request.session['snacks_list'] = foods     
            if(len(foods) == 0):  
                return render(request, 'users/snacks.html', dict2) 
...
def scrape(url):
    ...
    nutri.to_csv(r'C:\Users\sheki\OneDrive\Documents\MoM\AI_project\nutri.csv', index = False, header = False, mode = 'a')
    return nutri
